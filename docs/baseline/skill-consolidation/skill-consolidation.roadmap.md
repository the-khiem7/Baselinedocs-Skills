...
|---|---|---|---|---|
| SC-P1 | `sync-decision` folded into `sync-decisions` | SC-D2 | - | complete |
| SC-P2 | `maintain-prune` deleted, relocation rule in the contract | SC-D4, SC-D5 | SC-P1 | complete |
| SC-P3 | `audit-verify` renamed `audit-claims`, both audit descriptions rewritten | SC-D3 | - | complete |
| SC-P4 | `maintain-compact` carries the revert gate | SC-D7 | - | complete |
| SC-P5 | `maintain-archive` deleted, with `status: archived` and its only consumer | SC-D11 | SC-Q1 | complete |
| SC-P6 | `DESIGN.md`, `AGENTS.md`, `README.md` aligned to the end state, and the last duplicate rule statements removed | SC-D1, SC-D8, SC-D9, SC-D10 | SC-P1, SC-P2, SC-P3, SC-P4 | complete |
| SC-P7 | the sync and audit model corrected, and the routing pointers it exposed added | SC-D12 | SC-Q3 | complete |
| SC-P8 | installed skills replaced on this machine, and the stale-count class of defect given a test | SC-D8 | SC-P7 | complete |
| SC-P9 | the contract-shipping criterion changed to a full read, and `onboard` given the copy and a gate of its own | SC-D16 | - | complete |
| SC-P10 | report style shipped to every reporting skill, hard-wrapping banned, SC-Q6 and SC-Q7 opened from measured packs | SC-D17, SC-D18 | - | complete |
| SC-P11 | entry index built on this pack as a SC-Q7 trial | SC-Q7 | SC-P10 | complete |
| SC-P12 | the entry index required by the contract above a threshold | SC-D19 | SC-P11 | complete |
| SC-P13 | misfiled content given named owners in the contract, closing SC-Q5 | SC-D20 | SC-P11 | complete |
| SC-P14 | contract heading repaired, and the thread's uncaptured reasoning written down | SC-D21 | SC-P13 | complete |

## SC-P1: fold sync-decision into sync-decisions

Acceptance criteria:

- one skill covers both the single-decision and the multi-decision case
- no file in the repository names `baselinedocs-sync-decision` except this pack and `DESIGN.md`, which SC-P6 rewrites
...
| Residual references | `DESIGN.md` only, deferred to SC-P6 by design |
...
| `baselinedocs-sync-decisions/SKILL.md` | SC-D2's section-scope constraint added: patch only directly impacted sections, no broad unrelated rewrites. The mutual `Non-Goals` line pointing at the deleted skill removed. `description`, `Purpose`, `Use When`, `Core Behavior` also rewritten |
| `tests/test_references.py` | `"baselinedocs-sync-decision"` removed from `CONTRACT_SKILLS` |
| `README.md` | `Agent-Selected Skills` sync row |

Revision summary: the absorbed skill's trigger vocabulary was carried into the surviving description rather than dropped with the folder. Deleting a folder deletes a selection surface, and on hosts where `description` drives selection the words "atomic", "targeted", and "one specific" were the only route to the narrow case. This is part of the merge, not the separate description batch in SC-D9.

Revision summary: `README.md` is edited by each deletion phase for its own row, while SC-P6 keeps the counts, the criterion, and the deprecation record. SC-P1's acceptance criteria had required a clean repository-wide reference check that SC-P6 alone could not satisfy mid-sequence.

## SC-P2: delete maintain-prune, add the relocation rule

Acceptance criteria:

- the disproven-claim case has a named owner in the contract
- no file names `baselinedocs-maintain-prune` except this pack and the deprecation record SC-P6 adds to `DESIGN.md`
...
| Residual references | this pack only. `DESIGN.md` never named `maintain-prune`, so SC-P6 adds a deprecation record rather than repairing one |

Changes:

| File | Change |
|---|---|
| `contract/pack-contract.md` | new `Disproven claims` section carrying SC-D5's rule, placed after `Reasoning ownership`. 74 lines to 82 |
...
## SC-P3: rename audit-verify to audit-claims
...
| `baselinedocs-audit-claims/SKILL.md` | `name`, description rewritten on SC-D3's document-versus-claim axis, H1, `Core Behavior` step 3, new provenance rule, `Non-Goals` |
...
| `baselinedocs-maintain-compact/SKILL.md` | SC-D7's gate written in with its WHY clause as a new `Gate` section. Also description, `Core Behavior` step 1, deleted lesson-entry restatement, new `Non-Goals` pointer |

Three changes beyond the gate itself, each closing a hole SC-P2 opened or exposed:

`Core Behavior` step 1 said "repeated and low-value content". With `maintain-prune` deleted, "low-value" was the only phrase left in the family authorizing removal on grounds other than redundancy, which is the permission SC-P2 revoked at contract level. Now "repeated and redundant".
...
SC-P4 also invalidated one sentence in `DESIGN.md:190`, which credits `maintain-compact` with forbidding lesson-entry compression. The prohibition still exists and still holds; it now lives in the contract. SC-P6 owns that correction.

## SC-P5: delete maintain-archive and its orphaned consumers

Scope changed after SC-Q1 was answered. The phase was written as "specify the destination"; specifying it disproved SC-D6, and SC-D11 replaced it. Reasoning is in SC-D11, not here.
...
| `baselinedocs-maintain-compact/SKILL.md` | `Non-Goals`: `not for archiving completed phases` replaced with a direct prohibition on moving content out of the active pack, so the deletion does not silently grant compact the relocation right SC-D11 rejected |
| `README.md` | `Agent-Selected Skills` maintain row |
...
Revision summary: `AGENTS.md` still said 14 in two further places, at `The pack contract` bullets "copy it to all 14" and "Each of the 14". SC-P6 corrected the two prose mentions and missed these, because it searched for the sentence it knew about rather than for the number. All four now read 10.

Revision summary: the compact non-goal was replaced rather than deleted. It had named `maintain-archive` to mark a boundary, and deleting the line with the skill would have removed the prohibition along with the pointer, silently handing compact the relocation right SC-D11 records as rejected. Same shape as the SC-P6 pointer replacements: when a line names something being deleted, check whether the line is a pointer or a rule wearing a pointer's clothes.
...
## SC-P6: align the repository documents
...
Revision summary: the removal of the two duplicates SC-P4 found was added here rather than given its own phase, per SC-D10. The contract edit must land and be re-copied before either deletion, because both duplicates carry the after-resolution clause that the canonical sentence lacked.
...
| `DESIGN.md` | 261 to 366 lines. Three rows added to `Decision Summary`, the `resume-snapshot` justification corrected, the pointer criterion in `Deferred Work` replaced with SC-D1, deprecation records added for `sync-decision` and `maintain-prune`, line 190 corrected where it credited `maintain-compact` with a prohibition SC-P4 moved into the contract, and two new sections added, `Skill Consolidation` and `Detect And Repair` |
| `README.md` | already correct. The `Agent-Selected Skills` table had been updated by SC-P1, SC-P2, and SC-P3 as each landed, and the sections those edits did not touch carried no stale count or skill name |
...
`AGENTS.md` gained a pointer beyond the stated deliverables. Two of the alternatives rejected in SC-D1 are the kind a contributor would propose again on sight, and `AGENTS.md` is what a contributor reads before touching a skill folder, so a one-line pointer there is what makes the rejection reachable at the moment it matters.

## SC-P7: correct the sync and audit model, add the pointers it exposed

Opened when SC-Q3 was answered. No skill is merged, added, or renamed; the change is to the model and to four routing pointers. Reasoning is in SC-D12.
...
| SC-Q3 | closed by SC-D12. The namespace half of SC-Q2 closed with it |
...
## SC-P8: replace the installed skills, and pin the count
...
Deleting before installing was deliberate. The installer adds and overwrites but does not remove, so an install alone would have left all eight deleted skills in place, still competing for selection. That is the SC-D8 cost made concrete: a repository delete cannot reach an installed machine, and only an explicit delete on that machine can.
...
Revision summary: the pack was set to `status: complete` in this phase, while SC-Q2 and SC-Q4 were still open, on the grounds that neither was in its scope. Both were closed afterwards in SC-D13 and SC-D14 without touching a skill file, so the status needed no revisiting.

## SC-P9: ship the contract to the pack's only full reader

Opened by a real `onboard` run against an unrelated pack, which reported `onboard`'s absent contract copy as an unreadable file and concluded that future writes into that pack were blocked. Both halves were wrong, and the wrong conclusion was reachable because nothing recorded why the copy was absent. SC-D16 carries the argument.
...
The `Output` section's document names survived on purpose. They read as reporting order rather than as a second role list, and stripping them would have made the instruction vaguer without removing a fact. Recorded because the de-duplication argument was the strongest-sounding one on the table and it was the weakest in fact; the criterion argument in SC-D16 is what carried the phase.
...
Found while running SC-P9's gate, unrelated to SC-P9's change. Left in this phase rather than given its own, because a convention test that fails on foreign text is not a finding a later phase would inherit in a usable state.

## SC-P10: report style shipped, hard-wrapping banned, and two questions opened from measured packs

Four items reported from real use of the family against unrelated packs. Two were decided and implemented here, SC-D17 and SC-D18; two were opened as questions rather than answered, SC-Q6 and SC-Q7, because both need a wider sample or a design not yet argued.
...
Recorded because the hypothesis was mine and it was wrong, and because its failure is what makes SC-Q7 a structural question rather than an enforcement one. The measurements and the structural root cause both live in SC-Q7; they are not restated here.

## SC-P11: build the entry index on this pack as a SC-Q7 trial
...
- the table declares itself a trial under SC-Q7, so a later reader does not mistake it for a contract requirement
...
|---|---|
| Verification gate | 24 passed, 63 subtests passed |
| Rows | 21: SC-D1 through SC-D18, SC-Q5 through SC-Q7 |
| File cost, index alone | 317 lines and 41.8 KB to 347 lines and 44.3 KB. Plus 623 tokens. The document passed 370 lines once the trial write-up in SC-Q7 landed, and a figure measured inside the document it measures cannot stay exact |
...
| File | Change |
|---|---|
| `skill-consolidation.hallucination.md` | `Entry index` section added above SC-D1, with the rule that a row carries subject only and the note that it is a SC-Q7 trial |
| `skill-consolidation.hallucination.md`, SC-Q7 | trial result, the closed-to-open ratio measured across three journals, and the sequencing consequence it exposed |

The measurements and the reasoning both live in SC-Q7. Not restated here.

### Finding: what decides the payoff is placement, not size

The trial's useful output was not the saving on this pack. It was that the saving depends on the ratio of closed decisions to open questions, because only a closed entry can be deferred to a row, and that ratio varies from 86 percent closed to 46 percent closed across three real journals. The pack that most needs the mechanism benefits least, and its ratio is bad because settled material sits under its open-questions heading. That is SC-Q5 presenting as SC-Q7, and it changes the order the two should be answered in. Recorded in SC-Q7 with the numbers.

## SC-P12: the entry index enters the contract

The trial in SC-P11 produced numbers, and the numbers carried the decision. SC-D19 records the argument, the threshold's calibration, and the three rejected alternatives, one of which was this phase's own recommendation.
...
| Index rows here | 22, after SC-D19 got its own row |
...
| `<skill>/references/pack-contract.md` | all 11 copies re-synced |
| `skill-consolidation.hallucination.md` | index header now cites the contract instead of calling itself a trial; SC-D19 added; SC-Q7 narrowed to what `onboard` does with the index |
...
This phase recommended accepting the unverifiable row while still pinning the structural half, since tests for a missing row, a row pointing at no entry, and a row longer than one line are cheap. The user chose discipline alone. Implemented as decided and recorded in SC-D19 as a rejected alternative rather than as a disagreement, because the tests stay cheap to add later and because what they would not have caught is the only thing that matters here: whether a row's text is still true.

## SC-P13: misfiled content gets owners, closing SC-Q5

SC-Q5 had been left open through SC-P9 and SC-P10 on the grounds that answering it needed either a widened skill trigger or a fourth owner clause in the contract, and that neither was in scope. SC-P11's measurement overtook that: the repair returns more on a live pack than the mechanism SC-P12 shipped. SC-D20 carries the argument.
...
| SC-Q5 | closed by SC-D20. Its analysis moved into SC-D20 rather than being deleted with its heading |
...
| `<skill>/references/pack-contract.md` | all 11 copies re-synced |
| `baselinedocs-onboard/SKILL.md` | step 10 now names the owner the contract gives, and covers settled material under an unsettled heading. It previously said the finding had no owner and warned against inventing one, which SC-D20 made false |
| `skill-consolidation.hallucination.md` | SC-D20 added, SC-Q5's section removed with its content carried into SC-D20, three index rows repointed from SC-Q5 to SC-D20 |
...
|---|---|
| Verification gate | 24 passed, 63 subtests passed |
| `## Conditional documents` | restored. It had been destroyed by SC-P13's insertion, leaving `sourcecode` and `useguide` defined under `## Misfiled content` |
...
| `<skill>/references/pack-contract.md` | all 11 copies re-synced |
| `skill-consolidation.hallucination.md` | SC-D21 records the heading defect as a lesson entry; SC-Q8 opened for the within-entry label standard; SC-Q6 expanded with the full proposed scheme, its seven rules, and its four rejected alternatives |
...
The defect passed `cmp` across all 11 copies, which was accurate, and passed the suite at 24 tests. Both checks measure whether the copies agree with canonical. Neither can see a defect introduced into what they agree about, and the more copies there are the more reassuring a green result looks. SC-D21 states the class rather than the incident: a byte-identity test protects a fanout, not a source.
...
An earlier count in this thread reported SC-D8, SC-D9, and SC-D15 as closed decisions missing required labels. Reading all three showed SC-D15 is not a closed decision at all but a disproven-claim relocation, so the four-part rule does not apply to it and the count had used the wrong standard. SC-D8 carries the consequence under a different label name. SC-D9 is the only clear case. Corrected in SC-Q8, where it changed the shape of the question: a label standard has to enumerate entry kinds before it can be enforced, and it therefore costs more than it first appeared to.
...
|---|---|---|
| SC-Q1 archive destination | SC-D11 | specifying it disproved the skill; deleted with its enum value and consumer |
| SC-Q3 detect against repair | SC-D12 | the matrix was drawn on an axis that does not separate the five skills; none merged |
| SC-Q2 rename sweep | SC-D13 | stops at the audit pair. `sync-reconcile` is reached by pointer, not by name |
| SC-Q4 pack-level finished marker | SC-D14 | `onboard` step 3 already selects only a sub-pack in progress |

The SC-D9 description batch closes empty as well, for the reason in the Risks table above.

Three of the four closures reduced scope rather than adding it, and SC-D13 and SC-D14 changed no shipped file at all. What each leaves behind is the trigger that would justify reopening it, which is the part a later reader needs and the part a deleted question would not have carried.
